...
void swapLists(LList* llist1, LList* llist2, int threadNum) {
	Node *tmp_list;
	EnterCriticalSection(&(llist1->criticalSection));
	printf("inbetween the Enter Critical section %d\n", threadNum);
	EnterCriticalSection(&(llist2->criticalSection));
	tmp_list = llist1->pHead;
	llist1->pHead = llist2->pHead;
	llist2->pHead = tmp_list;
	LeaveCriticalSection(&(llist1->criticalSection));
	LeaveCriticalSection(&(llist2->criticalSection));
}
